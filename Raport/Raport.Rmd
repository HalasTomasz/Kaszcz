...
Nasze dane składają się z bardzo duzej liczby kolumn Wybraliśmy najważniejsze statystyki takie jak : \textit{life\_expectancy}, \textit{schooling} dla których następnie zostały wyliczone podstawowe parametry statystyczne. Wyniki przedstawiliśmy w poniższej \ref{tab:data} tabeli:
 ...
 \begin{table}[H]
      ...
      \begin{tabular}{|c|c|c|c|c|c|c|c|}
          \hline
           & \textit{life\_expectancy} &  \textit{schooling} \\
          \hline
          Minimum & 51.00  & 4.90 \\
          \hline
          1 Kwartyl? & 66.00  & 10.80 \\
          \hline
          Mediana & 73.90  & 13.10   \\
          \hline
          Średnia & 71.71 & 12.93   \\
          \hline
          3 Kwartyl? & 76.70  & 15.00\\
          \hline
...
Widzimy, że oczekiwana długość życia jest wynosi średnio około 72 lata. Natomiast parametr schooling oznacza się duża rozbieżnościa. Zauważyliśmy tutaj istneieje pewnej zależności liniowiej.  W swoim raporcie przyjżymy się korelacji  pomiędzy \textit{schooling}, a \textit{life\_expectancy}.
...
\section{Przedziały ufności} \label{section:ufnosc}
...
$$ \hat{\beta_0} \sim \mathcal{N} \left(\hat{\beta_0},\,\sigma^2 \left( \frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^n \left( x_i - \bar{x} \right)^2} \right) \right)\,. $$

$$ \hat{\beta_1} \sim \mathcal{N} \left(\hat{\beta_1},\,\frac{\sigma^2}{\sum_{i=1}^n \left( x_i - \bar{x} \right)^2} \right) \,. $$
Na podstawie powyższych informacji tworzymy przedziały ufności dla estymatorów $\hat{\beta_0}$ $\hat{\beta_1}$. Należy przy tym pamiętać, że dla nieznanego parametru $\sigma$, nasze estymatory mają własność:
$$
  \frac{\hat{\beta_0} - \beta_0}{S \sqrt{\frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^n \left( x_i - \bar{x} \right)^2}}} \sim t_{n-2}
$$
oraz

$$
  \frac{\hat{\beta_1} - \beta_1}{S \sqrt{\frac{1}{\sum_{i=1}^n \left( x_i - \bar{x} \right)^2}}} \sim t_{n-2}
$$
gdzie $S$ wynosi:
$$ S^2 = \frac{\sum_{i=1}^n (Y_i - \hat{Y} )^2}{n-2} $$
Korzystając z powyższych własności tworzymy przedziały ufności:

$$ P \left( -t_{\frac{\alpha}{2},n-2} < \hat{\beta_0} < t_{1 - \frac{\alpha}{2},n-2}  \right) =  1 - \alpha$$
rozkład $t$-studenta jest rozkładem symetrycznym, dlatego $t_{\frac{\alpha}{2},n-2} = t_{1-\frac{\alpha}{2},n-2}$. Następnie

$$ P \left( -t_{\frac{\alpha}{2},n-2} < \frac{\hat{\beta_0} - \beta_0}{S\sqrt{\frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^n (x_i - \bar{x})^2}}} < t_{\frac{\alpha}{2},n-2} \right) =  1 - \alpha$$
$$ P \left( \hat{\beta_0} -t_{\frac{\alpha}{2},n-2}S\sqrt{\frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^n (x_i - \bar{x})^2}} < \beta_0 < \hat{\beta_0} + t_{\frac{\alpha}{2},n-2}S\sqrt{\frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^n (x_i - \bar{x})^2}} \right) =  1 - \alpha$$
Otrzymaliśmy przedział ufności dla parametru $\beta_0$, gdzie $t_{\frac{\alpha}{2},n-2}$ jest kwantylem pochodzącym z rozkładu $t$-studenta z $n-2$ stopniami swobody. Analogicznie wyliczamy przedział ufności dla parametru $\beta_1$. Wygląda on następująco:

$$ P \left( \hat{\beta_1} -t_{\frac{\alpha}{2},n-2}S\sqrt{\frac{1}{\sum_{i=1}^n (x_i - \bar{x})^2}} < \beta_1 < \hat{\beta_1} + t_{\frac{\alpha}{2},n-2}S\sqrt{ \frac{1}{\sum_{i=1}^n (x_i - \bar{x})^2}} \right) =  1 - \alpha$$

Dla naszych dane przedziału ufnosci dla parametru $B_0$ wynoszą [41.707, 44,096] oraz dla $B_1$ [1.818, 2.64]

```{r przedział ufności}
  n = length(Y)
  S = sqrt(sum((Y - mean(Y))^2)/(n-2)) 
  varb0 = S * sqrt(1/n + mean(X)^2/sum((X-mean(X))^2))
  bLeft = b - qt(.975, df=n-2)*varb0
  bRight = b + qt(.975, df=n-2)*varb0
  
  varb1 = S * sqrt(1/sum((X-mean(X))^2))
  aLeft = a - qt(.975, df=n-2)*varb1
  aRight = a + qt(.975, df=n-2)*varb1
```

\section{Predykcja}

Aby dokonać predykcji będizemy chcieli dokonać estyamcji parametru $Y_0-\hat{Y_0}$, gdzie
$$Y_0 = \beta_0 + \beta_1 x_0 + \epsilon_0  $$

$$\hat{Y_0} = \mu_{Y_0} = \hat{\beta_0} + \hat{\beta_1}x_0  $$
Dla zmiennej losowej $Y_0-\hat{Y_0}$ wyliczamy jej wartośc oczekiwaną oraz wariancję:

$$ \mathrm{E}(Y_0-\hat{Y_0}) = \beta_0 + \beta_1 x_0 + \epsilon_0- \hat{\beta_0} - \hat{\beta_1}x_0 = 0  $$ 


$$ Var(Y_0-\hat{Y_0}) = Var(Y_0)+ Var(\bar{Y_0}) - 2Cov(Y_0,\bar{Y_0}) $$ 

Nasza $2Cov(Y_0,\bar{Y_0}) = 0$, ponieważ $Y_0$ oraz $\bar{Y_0}$ są od siebie niezależne.

$$ Var(Y_0-\hat{Y_0}) = Var(\beta_0 + \beta_1 x_0 + \epsilon_0)+ Var(\bar{Y} +\hat{\beta_1}(\bar{x}_0 -\bar{x}_n)) = \sigma^2 \left( 1 + \frac{1}{n} + \frac{(x_0 - x_n)^2}{\sum_{i=1}^n(x_i - \bar{x}_n)^2}  \right) $$ 

Dla powyżsych parametrów konstruowane są przedziały ufności. Ich konstrukcja jest analogiczna do tych przedstawionych w \ref{section:ufnosc}.
$$
 \frac{Y_0 - \hat{Y_0}}{S \sqrt{1 + \frac{1}{n} + \frac{(x_0 - x_n)^2}{\sum_{i=1}^n(x_i - \bar{x}_n)^2}}} \sim t_{n-2}
$$

$$
  P \left(\hat{Y_0} - t_{\frac{\alpha}{2},n-2} S \sqrt{1 + \frac{1}{n} + \frac{(x_0 - x_n)^2}{\sum_{i=1}^n(x_i - \bar{x}_n)^2}} \leq Y_0 \leq \hat{Y_0} + t_{\frac{\alpha}{2},n-2} S \sqrt{1 + \frac{1}{n} + \frac{(x_0 - x_n)^2}{\sum_{i=1}^n(x_i - \bar{x}_n)^2}} \right) = 1 - \alpha
$$
