...
Jak możemy zauważyć wyznaczona prosta pokrywa się z danymi.
...
|      W przypadku założenia drugiego najpierw obliczymy wariancję residuów korzystając z nieobciążonego estymatora postaci
$$ S = \frac{1}{n - 2} \sum_{i=1}^n e_i^2. $$
Dla naszych danych otrzymaliśmy $S \approx 20.93$. Następnie policzymy wariancje częściowe tj.
$$ S_k = \frac{1}{k - 2} \sum_{i=1}^k e_i^2, \ \ \ k = 3, \dots, n. $$

```{r res_wariancja, fig.cap="\\label{fig:res_wariancja} Porównanie wariancji częściowej $S_k$ residuów dla $k = 2,... , n$ z wariancją z całej próby, czyli $S$.", fig.align="center", fig.width = 6, fig.height = 3}
n <- length(E)
varE <- c()
for (i in 3:n) {
    varE <- append(varE, sum(E[1:i]^2) / (i - 2))
}
ggplot() + 
  geom_line(aes(3:n, varE, col='a')) + 
  geom_line(aes(3:n, sum(E^2) / (n - 2), col='b')) +
  scale_color_manual(labels=c(TeX("$S_k$"), TeX("$S$")), values=c('#00b3ff', "#d36f12")) +
  labs(col="") +
  xlab(TeX("$k$")) +
  ylab("Wariancja")
```

Na powyższym wykresie zobaczyć możemy, że wariancja częściowa wraz ze zwiększającym się $k$ szybko zbiega do wartości wariancji z całej próbki i oscyluje wokół niej. Na tej podstawie możemy zakładać, że $\mathrm{Var}(\epsilon_i)$ jest skończona.
...
```{r res_gestosc, fig.cap="\\label{fig:res_gestosc} Porównanie histogramu z danych z gęstością teoretyczną rozkładu $\\mathcal{N}(0, s)$.", fig.align="center", fig.width = 6, fig.height = 3}
